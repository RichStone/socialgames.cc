...
class Api::V1::MatchesControllerTest < Api::Test
  setup do
    ...
    @gaming_session = create(:gaming_session, team: @team)
    @match = build(:match, gaming_session: @gaming_session)
    @other_matches = create_list(:match, 3)

    @another_match = create(:match, gaming_session: @gaming_session)
  ...
  test "index" do
    ...
    get "/api/v1/gaming_sessions/#{@gaming_session.id}/matches", params: {access_token: access_token}
  ...
  test "create" do
    ...
    match_data = JSON.parse(build(:match, gaming_session: nil).api_attributes.to_json)
    match_data.except!("id", "gaming_session_id", "created_at", "updated_at")
    params[:match] = match_data

    post "/api/v1/gaming_sessions/#{@gaming_session.id}/matches", params: params
    assert_response :success
    ...
    # Also ensure we can't do that same action as another user.
    post "/api/v1/gaming_sessions/#{@gaming_session.id}/matches",
